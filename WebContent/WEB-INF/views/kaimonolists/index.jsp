        ...
        <form method="POST" action="<c:url value='/kaimonolists/create' />">
            ...
            <ul class="lists">
                <c:forEach var="kaimonolist" items="${kaimonolists}">
                    <li class="parent list-item"><c:out
                            value="${kaimonolist.ingredient}" /> <span class="close">×</span>
                        <input type="hidden" name="todoData"
                        value="${kaimonolist.ingredient }"> <input type="hidden"
            ...
            <div id="pagination">
                ...
                <c:forEach var="i" begin="1"
                    end="${((kaimonolists_count - 1) / 9) + 1}" step="1">
                    <c:choose>
                        <c:when test="${i == page}">
                            <c:out value="${i}" />&nbsp;
                    </c:when>
                        <c:otherwise>
                            <a href="<c:url value='/kaimonolists/index?page=${i}' />"><c:out
                                    value="${i}" /></a>&nbsp;
                    </c:otherwise>
                    </c:choose>
                </c:forEach>
